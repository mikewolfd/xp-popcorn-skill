...
The `SKILL.md` required. Other files are optional and let you build more powerful skills: templates for Claude to fill in, example outputs showing the expected format, scripts Claude can execute, or detailed reference documentation. Reference these files from your `SKILL.md` so Claude knows what they contain and when to load them. See [Add supporting files](#add-supporting-files) for more details.
...
```text  theme={null}
my-skill/
├── SK (required - overview and navigation)
...
Re from `SKILL.md` so Claude knows what each file contains and when to load it:
...
<html><head>
  ...
  <div class="container">
    <div class="sidebar">
      ...
      <div class="stat"><span>Direories</span><span class="stat-value">{stats["dirs"]:,}</span></div>
  ...
  <script>
    ...
    function render(node, parent) {{
      if (node.children) {{
        ...
        det.innerHTML = `<summary><span class="folder">📁 ${{nodeame}}</span><span class="size">${{fmt(node.size)}}</span></summary>`;
