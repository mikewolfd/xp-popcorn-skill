...
- ✅ **Peer-to-peer messaging** — Direct communication betwts (not just hierarchical)
...
> **📝 Documentation Update (2026-02-09)**: Architecture section corrected based on [Addy Osmani's research](https://addyosmani.eams/). Key clarification: Agents communicate via **peer-to-peer messaging** through a mailbox system, not only through team lead synthesis. Context windows remain isolated (1M tokens per agent), but explicit messaging enables direct coordination between teammates.
...
The ">5 agents" rule as a sensible default, but it breaks down in specific scenarios where the math favors larger teams. The real question is not "how many agents?" but "is the coordination overhead less costly than the context overflow?"
...
│ - Claims tasks │  │                 │ - Claims tas       │ - Messages     │
...
1s independently in its context
...
5. **Result synthesis**: Team lead collects finings and presents unified response
...
- **Teammate ↔ te**: Direct peer-to-peer messaging (challenge approaches, debate solutions)
...
├─ Teammate 1 (Security): Analyzes → Messages Teammate 2: "Found auth issue in line 4Teammate 2 (Code Quality): Reviews → Messages back: "Confirmed, also see OWASP violation"
...
- ✅ Coordintly (self-organization)
- ✅ Share discoveries mid-workflow (via messages, not context)

**Limitation**: Context isolation rains—agents don't share their full context window, only explicit messages.

### Navigation Between Agents

**Built-in navigation**:
- **Shift+Down**: Cycle through teammates in in-process mode
- **tmux/iTerm2**: Split pane mode with `teammateMode: "tx"` (requires tmux or iTerm2 with `it2` CLI)
...
- ✅ **Messages ARE shared**: Agents senplicit messages via mailbox (findings, questions, debates)
...
ho 'export CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1' >> ~/.bashrc
source ~/.bashrc
```
...
**After editing**, restde Code for changes to take effect.
...
**Why it worked**:
- **Read-heavy task**: Code review = primarily reading/anal(no write conflicts)
...
- ✅ Maintained compliance (financial services sards)
...
- **Tight deadlines**: Speed requireme justified token cost
...
**Why it worked**:
- r phases**: Each compiler phase is well-defined (lexer → parser → codegen)
...
> â a C compiler with agent teams](https://www.anthropic.com/engineering/building-c-compiler), Anthropic Engineering, Feb 2026

**Key learnings**:
- ⚠️ **Tests passing ≠ correctness**: oversight still important for quality assurance
- ⚠️ **Verition required**: Automated success doesn't guarantee error-free code
- ✅ **Feasibility proven**: Complex multi-phase projects achievable with agent teams

**Source**: [Building a C compil with agent teams](https://www.anthropic.com/engineering/building-c-compiler), Anthropic Engineering, Feb 2026
...
**Source**: [Paul RayneLinkedIn](https://www.linkedin.com/posts/thepaulrayner_this-is-wild-i-just-upgraded-claude-code-activity-7425635159678414850-MNyv), Feb 2026
...
- Time-critical debug (production issues)
...
**Benefits**:
- ✅ **Context preservation**: Al47 files in one coordinated session (vs losing context after ~15)
- ✅ **Interface consistency**: Shared contracts enforced across agents
- ✅ **Atomic migration**: All layers updated in coordination

**Gotcha**:
- ⚠️ **Merge conflicts**: If ageny same files (e.g., shared types)
...
**Context**: What changes when using agent teams vs single-agenssions?
...
### Cost Trade-**Token consumption comparison** (estimated):
...
- ❌ **Budget-constrained**: Personal projects with tight token limitof thumb**: Agent teams justified when time saved > 2x token cost increase.
...
- Code review: Agents readode, provide analysis
...
- Refactoring shared types: Multiple agents modify same file → merge confl Database schema changes: Coordinated migrations across files
...
- ✅ferent functions in same file (clean git merges)
- ✅ Additive changes (new functions, no edits)

**Automatic resolution struggles**:
- ❌ Same lines modified (classic merge conflict)
- ❌ Conflicting logic (Agent A removes validation, Agent B adds ❌ Circular dependencies (Agent A needs Agent B's output, vice versa)
...
// Conflict: Both mified same function
...
- ✅ Complexity justifies parallelization (muyer analysis)
...
- ⚠️ **Performance variability**: Coordination speed may fluctmentation evolving**: Official docs still minimal
...
- ✅ CRED: 2x speed sers, financial services)
- ⚠️ Community: Mixed reports (some merge conflict issues)

### Context Isolation

**What agents can't do**:
- ❌ **Share context windows**: Agent 1's full co(1M tokens) not visible to Agent 2
...
- ✅ **Challenge approaches**: Debate solutions, ask questeach other
- ✅ **Share findings**: Explicit messaging (not automatic context sharing)

**Implications**:
```
Scenario: Agent discovers critical bug that affects Agent 2's work
...
- ❌ **Sequential dependencisk B requires Task A completion (no parallelization benefit)
- ❌ **Budget constraints**: Personal projects, learning (token cost multiplier)
- ❌ **Tight interdependencies**: Circular deencies between tasks
...
Start
  │
  ├─ Task is simple (<5 files)? ──YES──> Single agO
  │
...
- Full codebase anasis (architecture review)
...
- âent agent communications (audit trail)

**When to use Agent Teams**:
- ✅ Cloud development (Anthropic API access)
- ✅ Rapid setup (no infrastructure required)
- ✅ Git-native workfloweady using git)
- ✅ Official support path (Anthropic-maintained)

**Open question** (as of Feb 2026):
> "I'm not sure about Claude's guidance on when to use beads versus agent team sessio."
...
Team lead
  ...
  └─ Agent 2 (Frontend lead)
      ├─ Agent 2a: Components
      └─ Agent 2b: State managemmd for Compound Learning

Agent teams benefit from a shared context file that accumulates cross-session learnings — patterns that worked, pitfalls to avoid, codebase-specific gotchas. This file is called `AGENTS.md` (analogous t`CLAUDE.md` but scoped to agentic workflows).
...
- Linter ron save; do not commit with lint errors, the CI gate is strict

## Style
- All API responses must follow the ApiResponse<T> wrapper type
- Error codes live in src/constants/errors.ts — alys reuse, never hardcode strings
```

**Critical rule — never let agents write AGENTS.md directly**. ETH Zürich research (Gloaguen et al., 2026) confirms that LLM-generated context filesduce task success by ~3% and increase inference costs by 20%+, compared to a ~4% improvement from developer-written files. The mechanism: agents generate generic, bloated context that creates cognitive overhead for every subsequent agent reading it.
...
**Maintenance**: Review AGENTS.md after each team session (Retro step of the Factory Model). Remove entries that are no lonr relevant — stale instructions are actively harmful, not neutral.

### Git Worktree Management

**Why worktrees matter**:
- Each agent works in separate git worktree (isolated file syste
...
- ✅ Descriptive branch names (`agent1-backend-api`, `agent2-froui`)
...
1. **Lazy spawning**: Only spawn agents when parallelization carly benefits
   ```
...
5. **Hard token budgets per agent**: Assign domain-specific limits to prevent runaway consumption
   ...
   ```
   Token costs scale linearly with team size — a agent team can consume 5× the tokens of a single session. Caps prevent one agent's rabbit hole from blowing the entire session budget.

### Quality Assurance

**Validation checklist**:
- [ ] **All agents completed**: No hanging tasks
- [ ] **Merge conlicts resolved**: Clean git history
...
The mandatory reflection prompt ("What failed? What specific change would fix it?") reduces stuck agents substantially — it forces the agent to change approach rather than repeat the se failing action with minor variations.

**Kill and reassign criteria**:
- Stuck 3+ iterations on the same blocker → kill the task, reassign with more specific context
- Task consumed85% of its token budget with no commit → pause and report
- No progress after 2 reflection cycles → escalate to team lead

### Dedicated Reviewer Teammate

For production agent teams, adding a read-only reviewer agent improves output qualitthout slowing throughput:
...
- Trigger: tomatically review on every TaskCompleted event
...
Cycle 3: Agent receives final context
         → Produce best output regardlemaining uncertainty
         → Flag explicit assumptions made
```

### What to Pass Sub-Agents

The most common mistake: giving a sub-agent the WHAT without the WHY. An agent that knows is "implementing a retry mechanism for the payment service" has context that saves correction cycles:

```markdown
## Objective
[WHY this task exists — the problem being solved, the constrnt being met]
...
|-----------|------------------------|
| Sub-agt modifies 1–2 known files | No — provide the files directly |
| Sub-agent needs to understand system behavior | Yes — it may need to trace call graphs |
| Sub-agent makes architecturisions | Yes — always |
| Sub-agent writes tests for existing code | Often — it needs to read what it's testing |

The overhead is real (each cycle costs tokens and latency). Apply it to tasks where wrong assumptions would cost more than the retrie— typically anything touching interfaces, contracts, or public APIs.

> **Credit**: Iterative retrieval pattern for sub-agents from [Everything Claude Code](https://github.com/affaan-m/evything-claude-code) (Affaan Mustafa). The max-3-cycles bound and the WHY/WHAT separation are documented in their longform guide.
...
- [Sub-Agents](#split-role-s-agents) — Single-agent task delegation
- [Multi-Instance Workflows](#917-scaling-patterns-multi-instance-workflows) — Manual parallel coordination
- [Dual-Instance Pattern](#alternativttern-dual-instance-planning-vertical-separation) — Plan-execute split
- [AI Ecosystem: Beads Framework](../ecosystem/ai-ecosystem.md#beads-framework) — Alternative orchestration (Gas T
