...
Vendored lead reference for **OpenAI Codex** with the `platforms/codex/subagent/` source tree and generated `.codex/` layout. **Default `subagent` mode** (file bus). For hook behavior and project-root resolution, see **`COMPANION.md`**.
...
4. **`printf 'subagent\n' > .popcorn-xp/{team}/.runtime-mode`** (or omit **`.runtime-mode`** if your **`shared/runtime/bin/session`** defaults to subagent).
...
- **`session close-check`** → append **`RETRO.md`** (≥5 real lines) → **`session close`** (or **`close --force`** only if you accept skipping gates). Successful **`close`** clears **`.popcorn-xp/.active-team`** (when it still names this team) and truncates **`context-store.log`**; run **`echo {team} > .popcorn-xp/.active-team`** again before the next slice. In each new **RETRO.md** entry, set **Lead host:** **`codex`** and **Task transport:** **`subagent`** (or **`team`** if you opted in) so future readers know which playbook and hooks applied — same fields as **`platforms/claude/subagent/skills/popcorn-xp/SKILL.md`** retro template.
...
The full lead playbook (team vs subagent, native Agent Teams, coordinator mode) is **`platforms/claude/subagent/skills/popcorn-xp/SKILL.md`** in the **Claude Code plugin** tree — not required on disk for this Codex-only bundle.
