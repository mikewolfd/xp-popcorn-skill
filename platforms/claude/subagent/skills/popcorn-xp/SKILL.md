...
Launch an XP pair-programming session. You (the lead) set up the session and step back. **Default to `subagent` mode:** teammates coordinate through **durable files** and `shared/runtime/bin/session` (task bus, chat, hooks as documented). **Team mode** uses **SendMessage** between teammates - only when the user explicitly wants native Agent Teams; it is **discouraged for now** because of **severe token inflation from a Claude Code bug** in that path. One **driver** (current), one **navigator** (future), one **advisor** (past); driver and navigator rotate; OBJECTIONs still block completion.
...
The plugin ships with agent definitions in `platforms/claude/subagent/agents/` that can be used as teammates. The default team is **3 core members**: **one driver, one navigator, one advisor** - **current, future, and past** respectively. The driver implements what is in flight now; the navigator reads ahead and steers; the advisor reviews what has already landed (verification, regressions, objections). Map concrete personas from `platforms/claude/subagent/agents/` onto those three seats. Add specialists for specific tasks; they are supplemental, not part of the core rotation unless you promote them.
...
Glob("{project}/platforms/claude/subagent/agents/*.md")
```
