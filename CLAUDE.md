...
**Claude Code:** Agents in **`platforms/shared/agents/`** (symlinked under each Claude plugin) use Markdown with YAML frontmatter: `name`, `description`, `model`, `color`, `skills: [popcorn-xp-protocol]`, plus lens text. The `skills` field auto-loads the protocol into each agent's context.
