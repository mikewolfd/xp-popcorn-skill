...
# when not in a repo). See hooks/scripts/px-resolve-claude-project-dir.sh.
...
SESSION_BIN="${CLAUDE_PROJECT_DIR}/bin/session"
CTX="Popcorn XP: active team \"${TEAM}\" — session root ${CLAUDE_PROJECT_DIR}/.popcorn-xp/${TEAM}. Use \"${SESSION_BIN}\" (or .popcorn-xp/${TEAM}/session) for LOG, ADVICE, state, and task bus. Subagent: set .runtime-mode to subagent. Vendored notes: codex/COMPANION.md."
